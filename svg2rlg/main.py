...
from .shapes import ShapeConverter
from .utils import node_attrs
...
class SvgRenderer:
    ...
    def renderSvg(self, node):
        ...
        """
        width, height = node_attrs(node, "width", "height")  # list(map(node.getAttribute, ("width", "height")))
...
def svg2rlg(path):
    ...
    try:
        svg = parse(path).documentElement
    except:
        print("Failed to load input file!")
        return

    # convert to a RLG drawing
    renderer = SvgRenderer(path)
    renderer.render(svg)
